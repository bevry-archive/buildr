	...
	Buildr.prototype.extend({
		...
		loadOptions: function(rootPath){
			...
			if ( exists ) {
				...
				options.extend(JSON.parse(fs.readFileSync(config.metaPath).toString()).buildr||{});
			}
		...
		applyOptions: function(options){
			...
			var expandPath = function(key,value,relativePath){
				...
				// Relative to Relative Path
					...
					value = value.replace(/^\.\//, relativePath);
				}
			...
			// Templates
			config.templates.each(function(key,value){
				var templateData =  (value && path.exists(value)) ? fs.readFileSync(value).toString() : '';
		...
		bundleJs: function(){
			...
			var
				config = this.config,
				bundleSrcData = '', bundleSrcPath = config.dir.src+'/'+config.bundle.js,
				bundleOutData = '', bundleOutPath = config.dir.out+'/'+config.bundle.js;
			...
			config.subpackages.each(function(i,subpackage){
				...
				bundleSrcData += config.templateContent['src-bundle-subpackage.js']
					.replace('%SUBPACKAGE%',path.basename(subpackage));
			...
			if ( config.bundle.src ) {
				console.log("\nBundling JS Files to: "+bundleSrcPath);
		...
		bundleCss: function(){
			...
			var
				config = this.config,
				bundleSrcData = '', bundleSrcPath = config.dir.src+'/'+config.bundle.css,
				bundleOutData = '', bundleOutPath = config.dir.out+'/'+config.bundle.css;
			...
			fs.writeFileSync(bundleOutPath,bundleOutData);
			if ( config.bundle.src ) {
				console.log("\nBundling CSS Files to: "+bundleSrcPath);
				fs.writeFileSync(bundleSrcPath,bundleSrcData);
			}
		},
